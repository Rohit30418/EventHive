...
const Events: React.FC = () => {
  ...
  const { upcomingEvents, pastEvents, hasResults } = useMemo(() => {
    if (!data || !Array.isArray(data) || authLoading || !user) {
  ...
  }, [data, searchTerm, user, role, authLoading]);
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-slate-500 dark:text-slate-400">
              {role === "SuperAdmin" ? "Managing all platform events from one place." : "Manage your events, details and public pages."}
            </p>
          </div>
      ...
      {!hasResults ? (
        <div className="eh-card flex min-h-[360px] flex-col items-center justify-center rounded-[2rem] border-dashed p-8 text-center">
          ...
          <p className="mt-2 max-w-md text-slate-500 dark:text-slate-400">
            {role !== "SuperAdmin" ? "Create your first event or adjust your search query." : "No platform events match the current search."}
          </p>
        </div>
      ) : (
